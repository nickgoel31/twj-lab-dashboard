...
const safeParse = (data: string | string[] | null | undefined): string[] => {
  if (!data) return [];
...
const PricingEditor = ({ pricingData, setPricingData }: PricingEditorProps) => {
  ...
  const handleFeature = (planId: number, type: 'features' | 'featuresNotIncluded', action: 'add' | 'remove', value?: string, index?: number) => {
    ...
    // 1. Parse existing string to array
    const rawData = category.plans[planIndex][type];
    const currentList = safeParse(rawData as string);

    if (action === 'remove' && typeof index === 'number') {
        currentList.splice(index, 1);
    } else if (action === 'add') {
        currentList.push("New Feature");
    }

    // 2. Stringify back to store in state (to match Prisma type)

    category.plans[planIndex][type] = JSON.stringify(currentList);
  ...
  const handleFeatureEdit = (planId: number, type: 'features' | 'featuresNotIncluded', index: number, text: string) => {
      ...
      if(plan) {
          // 1. Parse
          const currentList = safeParse(plan[type] as string);
          ...
          plan[type] = JSON.stringify(currentList);
  ...
  // ✅ FIX: Parse -> Modify -> Stringify
  const handlePasteFeatures = (e: React.ClipboardEvent<HTMLInputElement>, planId: number, type: 'features' | 'featuresNotIncluded', index: number) => {
    const paste = e.clipboardData.getData('text') || '';
    if (!/\r?\n/.test(paste)) return; // Allow normal paste if no newlines
    ...
    if (index >= 0 && index < currentList.length) {
      currentList.splice(index, 1, lines[0]);
      if (lines.length > 1) {
        currentList.splice(index + 1, 0, ...lines.slice(1));
      }
    } else {
    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="space-y-4">
        {activeCategory && activeCategory.plans.map((plan) => {
          ...
          // ✅ FIX: Parse arrays for rendering inputs
          const featuresList = safeParse(plan.features as string);
          const notIncludedList = safeParse(plan.featuresNotIncluded as string);
              ...
              >
               <div className="p-5 border-t border-slate-100 space-y-4">
                ...
                <div className="space-y-2 pt-2">
                    ...
                    {/* ✅ FIX: Map over the parsed list */}
                    {featuresList.map((feat, idx) => (
                        <div key={idx} className="flex gap-2">
                            <input
                                value={feat}
                                onChange={(e) => handleFeatureEdit(plan.id, 'features', idx, e.target.value)}
                                onPaste={(e) => handlePasteFeatures(e, plan.id, 'features', idx)}
